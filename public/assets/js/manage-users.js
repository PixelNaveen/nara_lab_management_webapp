...
// === DOM ELEMENTS ===
const manageUsersModalOverlay = document.getElementById(
...
const manageUsersBtnCloseModal = document.getElementById(
  "manageUsersBtnCloseModal",
);
...
const manageUsersDeleteModal = document.getElementById(
  "manageUsersDeleteModal",
);
const manageUsersToastContainer = document.getElementById(
  "manageUsersToastContainer",
);
...
function loadUsers() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="7" class="text-center text-muted"><i class="fas fa-spinner fa-spin"></i> Loading users...</td></tr>';

  sendUserAjax("fetchAll", {})
    .then((res) => {
      tbody.innerHTML = "";
...
const inputConfirmPassword = document.getElementById(
  "manageUsersConfirmPassword",
);
const checkChangePassword = document.getElementById(
  "manageUsersChangePasswordCheck",
);
...
const errConfirmPassword = document.getElementById(
  "errManageUsersConfirmPassword",
);

// Event Listeners for real-time validation
inputFullName.addEventListener("input", () =>
  validateInput(inputFullName, errFullName, {
    required: true,
    regex: nameRegex,
...
inputUsername.addEventListener("input", () =>
  validateInput(inputUsername, errUsername, {
    required: true,
  }),
);
inputEmail.addEventListener("input", () =>
  validateInput(inputEmail, errEmail, {
    required: true,
    regex: emailRegex,
...
inputRole.addEventListener("change", () =>
  validateInput(inputRole, errRole, {
    required: true,
  }),
);
...
function validateConfirmPassword() {
  if (!inputConfirmPassword.required && !inputConfirmPassword.value)
    return true;
  let isValid = true;
  let msg = "";
...
function openUserModal(mode) {
  ...
  if (mode === "create") {
    ...
    document
      .querySelector(".manage-users-password-fields")
    ...
    document
      .getElementById("changePasswordContainer")
      .classList.remove("d-none");
...
function attachUserRowEvents() {
  ...
  document.querySelectorAll(".btn-delete").forEach((btn) => {
    btn.onclick = (e) => {
      ...
      document.getElementById("manageUsersDeleteUserName").textContent =
...
manageUsersBtnUpdate.onclick = async () => {
  ...
  const passwordChanged =
    checkChangePassword.checked && inputPassword.value !== "";
  const changed = Object.keys(data).some(
    (key) =>
      data[key] !== manageUsersOriginalData[key] &&
      key !== "password" &&
      key !== "user_id",
  );
...
// === FILTERS ===
const manageUsersSearchInput = document.getElementById(
  "manageUsersSearchInput",
);
const manageUsersRoleFilter = document.getElementById("manageUsersRoleFilter");
const manageUsersStatusFilter = document.getElementById(
...
function applyUserFilters() {
  ...
  rows.forEach((tr) => {
    ...
    const combined =
      `${tr.dataset.fullname} ${tr.dataset.username} ${tr.dataset.email}`.toLowerCase();
  ...
  const noResultsRow = document.querySelector(
    "#manageUsersTable tbody tr.no-results",
  );
  if (visibleCount === 0) {
    if (!noResultsRow) {
      document
        .querySelector("#manageUsersTable tbody")
        .insertAdjacentHTML(
          "beforeend",
