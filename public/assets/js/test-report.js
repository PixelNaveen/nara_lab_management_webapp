...
const TestReport = (() => {
  ...
  // ==================== CONFIGURATION ====================
  const API_URL = "src/Controllers/test-report-controller.php";
  let currentStep = 1;
  let reportData = null;
  let allSignatories = [];
  ...
  function renderSamplesTable(samples) {
    ...
    tbody.innerHTML = samples
      .map((s) => {
        ...
        const receivedDate = s.received_date
          ? new Date(s.received_date).toLocaleDateString("en-GB", {
              day: "2-digit",
              month: "short",
              year: "numeric",
            })
        ...
        return `<tr>
                <td class="px-3 py-2 fw-medium">${escHtml(s.sample_code)}</td>
                <td class="px-3 py-2">${escHtml(s.client_name)}</td>
                <td class="px-3 py-2 text-center">${s.item_count}</td>
                <td class="px-3 py-2 text-center">${s.result_count}/${s.test_count}</td>
                <td class="px-3 py-2"><small>${receivedDate}</small></td>
                <td class="px-3 py-2 text-center">${reportBadge}</td>
