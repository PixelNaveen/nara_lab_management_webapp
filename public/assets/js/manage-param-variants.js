...
const btnCloseVariantModal =
  variantModalOverlay.querySelector(".btn-close-modal");
...
const btnCloseDeleteVariantModal =
  deleteVariantModal.querySelector(".btn-close-modal");
...
const inputSearch = document.querySelector(
  '.variants-card-filter input[type="text"]',
);
const selectFilterParameter = document.getElementById("filterParameter");
const selectStatus = document.querySelectorAll(
  ".variants-card-filter select",
)[1];
...
function showToastVariant(message, type = "success") {
  ...
  const toastContainer =
    document.getElementById("variantToastContainer") || document.body;
...
function openVariantModal(editData = null) {
  ...
  if (editData) {
    ...
    document.getElementById("variantStatus").value =
      editData.is_active == 1 ? "active" : "inactive";
...
document
  .getElementById("variantParameter")
  .addEventListener("change", function () {
    validateVariantInput(
      this,
      document.getElementById("variantParameterError"),
    );
  });
...
variantForm.addEventListener("submit", (e) => {
  ...
  const vParam = validateVariantInput(
    document.getElementById("variantParameter"),
    document.getElementById("variantParameterError"),
  );
  const vName = validateVariantInput(
    document.getElementById("variantName"),
    document.getElementById("variantNameError"),
  );
  const vStatus = validateVariantInput(
    document.getElementById("variantStatus"),
    document.getElementById("variantStatusError"),
  );
  ...
  const data = {
    ...
    is_active:
      document.getElementById("variantStatus").value === "active" ? 1 : 0,
  };
...
btnFilter.onclick = () => {
  const filters = {};
  if (selectFilterParameter.value)
    filters.parameter_id = selectFilterParameter.value;
