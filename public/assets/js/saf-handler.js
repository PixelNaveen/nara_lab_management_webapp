...
async function downloadPDF() {
  ...
  try {
    ...
    if (pageCount > 3) {
      console.log(
        `📄 Generating ${pageCount} pages... This may take 30-60 seconds.`,
      );
    }
    ...
    alert(
      "❌ Error generating PDF. Please try the browser print function instead (Ctrl+P) and save as PDF.",
    );
    ...
    if (btnDownload) {
      setTimeout(() => {
        ...
        if (
          !btnDownload.textContent.includes("Downloaded") &&
          !btnDownload.textContent.includes("Error")
        ) {
          btnDownload.textContent = "📥 Download as PDF";
          btnDownload.style.background = "";
        }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const pageCount = document.querySelectorAll(".form-container").length;
  console.log(
    `📄 SAF loaded with ${pageCount} page${pageCount > 1 ? "s" : ""}`,
  );

  if (pageCount > 5) {
    console.warn(
      `⚠️ Large form (${pageCount} pages). PDF generation may take time.`,
    );
  }
  ...
  if (typeof html2pdf === "undefined") {
    console.error(
      "❌ html2pdf library not loaded! PDF download will not work.",
    );
    console.error(
      "Please ensure /public/assets/libs/html2pdf.bundle.min.js is present.",
    );
