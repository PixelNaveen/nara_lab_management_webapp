...
const ResultEntry = (function () {
  ...
  // ==================== CONFIGURATION ====================
  const CONFIG = {
    CONTROLLER_URL: "../../src/Controllers/result-entry-controller.php",
    DEBOUNCE_DELAY: 500,
    TOAST_DURATION: 4000,
  };
  ...
  function createRow(s) {
    ...
    return (
      ...
      s.sample_id +
      '">' +
      '<td class="px-3 py-3"><span class="fw-semibold text-primary">' +
      esc(s.sample_code) +
      "</span></td>" +
      '<td class="px-3 py-3 client-name-column"><span class="fw-medium">' +
      esc(s.client_name) +
      "</span>" +
      (s.city_name
        ? '<br><small class="text-muted">' + esc(s.city_name) + "</small>"
        : "") +
      "</td>" +
      ...
      esc(s.status) +
      "</span></td>" +
      '<td class="px-3 py-3 text-center"><span class="fw-medium">' +
      s.item_count +
      "</span></td>" +
      '<td class="px-3 py-3 text-center">' +
      progress.html +
      "</td>" +
      '<td class="px-3 py-3"><span class="text-muted">' +
      date +
      "</span></td>" +
      '<td class="px-3 py-3 text-center">' +
      '<div class="d-flex gap-1 justify-content-center">' +
      (!isCompleted && !isCancelled
        ? '<button class="btn btn-sm btn-primary re-action-btn btn-assign" data-sample-id="' +
          s.sample_id +
          '" title="Enter Results"><i class="bi bi-pencil-square"></i> Assign</button>'
        : '<button class="btn btn-sm btn-outline-primary re-action-btn btn-view-results" data-sample-id="' +
          s.sample_id +
          '" title="View Results"><i class="bi bi-eye"></i> View</button>') +
      '<button class="btn btn-sm btn-outline-secondary re-action-btn btn-print" data-sample-id="' +
      s.sample_id +
      '" title="Print Results"><i class="bi bi-printer"></i></button>' +
      "</div></td>" +
      "</tr>"
    );
  }
  ...
  function attachRowListeners() {
    ...
    document.querySelectorAll(".btn-print").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        window.open(
          "/src/Controllers/forms-controller.php?action=view&sample_id=" + sid,
          "_blank",
  ...
  function renderFormModal(data) {
    ...
    var html =
      ...
      '<div class="col-md-4">' +
      '<div class="re-info-chip re-date-chip">' +
      '<label for="reAnalysisStartDate" class="form-label mb-1 text-muted" style="font-size:0.75rem;"><i class="bi bi-calendar-event me-1"></i>Analysis Start Date</label>' +
      '<input type="date" class="form-control form-control-sm" id="reAnalysisStartDate" value="' +
      (sample.analysis_start_date || "") +
      '">' +
      "</div></div>" +
      '<div class="col-md-4">' +
      '<div class="re-info-chip re-date-chip">' +
      '<label for="reAnalysisEndDate" class="form-label mb-1 text-muted" style="font-size:0.75rem;"><i class="bi bi-calendar-check me-1"></i>Analysis End Date</label>' +
  ...
  function saveResults() {
    ...
    rows.forEach(function (row) {
      ...
      if (!resultType) return; // skip empty
  ...
  function showToast(message, type) {
    ...
    var bgMap = {
      ...
      info: "linear-gradient(135deg, #3b82f6 0%, #1e3a8a 100%)",
    };
    ...
    toast.innerHTML =
      '<div class="re-toast-body"><i class="bi ' +
      (icons[type] || icons.info) +
      '"></i><span>' +
      esc(message) +
      "</span></div>" +
      '<button class="re-toast-close" onclick="this.parentElement.remove()">×</button>';
