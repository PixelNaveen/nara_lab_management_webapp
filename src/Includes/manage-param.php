...
// Load active test methods (modern fetch)
...
async function editParameter(id) {
    ...
    if (result.status === 'success') {
        ...
        swabPriceRow.style.display = 'none';
    } else showToast(result.message || 'Failed to load parameter', 'danger');
}
...
parameterForm.addEventListener('submit', async (e) => {
    ...
    method_id: document.getElementById('paramMethod').value,
    ...
    if (!data.parameter_name) { showToast('Parameter name is required', 'warning'); return; }
    ...
    } else showToast(result.message || 'Failed to save parameter', 'danger');
});
...
searchInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') btnFilter.click(); });
