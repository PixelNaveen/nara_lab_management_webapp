...
<script>
  ...
  function renderTable(data) {
    if (!data || data.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="text-center text-muted">No parameters found</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = data
      .map(
        ...
        <tr data-id="${p.parameter_id}">
            ...
            <td><span class="badge-status bg-${
              p.swab_enabled == 1 ? "success" : "secondary"
            }">
                ${p.swab_enabled == 1 ? "Enabled" : "Disabled"}</span></td>
            <td>${p.variant_count || 0}</td>
            <td><span class="badge-status bg-${
              p.is_active == 1 ? "success" : "secondary"
            }">
                ${p.is_active == 1 ? "Active" : "Inactive"}</span></td>
            
            <td>
                <button class="btn-parameters-edit" data-id="${
                  p.parameter_id
                }" title="Edit">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn-parameters-delete" data-id="${
                  p.parameter_id
                }" title="Delete">
  ...
  async function loadParameters(page = 1) {
    ...
    tbody.innerHTML =
      '<tr><td colspan="9" class="text-center"><div class="spinner-border text-primary"></div></td></tr>';
  ...
  function attachRowEvents() {
    document
      .querySelectorAll(".btn-parameters-edit")
      .forEach((btn) =>
  ...
  parameterForm.addEventListener("submit", async (e) => {
    ...
    formData.append(
      "parameter_name",
      document.getElementById("paramName").value.trim()
    );
    formData.append(
      "parameter_category",
      document.getElementById("paramCategory").value.trim()
    );
    formData.append(
      "base_unit",
      document.getElementById("paramBaseUnit").value.trim()
    );
    ...
    const selectedMethods = Array.from(methodSelect.selectedOptions).map(
      (opt) => opt.value
    );
  ...
  paramSwab.addEventListener("click", () => {
    const mode = document.getElementById("formMode").value;
    swabPriceRow.style.display =
      mode === "create" && paramSwab.value === "1" ? "block" : "none";
  });
